...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto flex min-h-screen flex-col sm:max-w-2xl sm:flex-row-reverse sm:gap-x-4 sm:px-4">
      <main className="border-mauve-dim flex-1 overflow-hidden pb-5 sm:border-x">{children}</main>

      <nav className="sticky bottom-0 sm:static">
          ...
          {items.map(({ href, segment, icon, text }) => {
            return (
              <ActiveLink
                ...
                href={href}
                segment={segment}
                className="group bg-mauve-app flex-1 select-none outline-none sm:-ml-3 sm:py-1"
